...
import cn.modificator.launcher.BubbleTextView.BubbleTextShadowHandler;
import cn.modificator.launcher.R;
import cn.modificator.launcher.LauncherSettings.Favorites;
import cn.modificator.launcher.accessibility.DragAndDropAccessibilityDelegate;
import cn.modificator.launcher.accessibility.FolderAccessibilityHelper;
...
import cn.modificator.launcher.util.CellAndSpan;
import cn.modificator.launcher.util.GridOccupancy;
import cn.modificator.launcher.util.ParcelableSparseArray;
...
public class CellLayout extends ViewGroup implements BubbleTextShadowHandler {
    ...
    public CellLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        mBackground = (TransitionDrawable) res.getDrawable(R.drawable.bg_celllayout, context.getTheme());
            ...
            switch (whichEdge) {
                case LEFT:
                    for (int i = cs.cellY; i < cs.cellY + cs.spanY; i++) {
                        if (i > leftEdge.length || leftEdge[i] == cs.cellX + cs.spanX) {
                            return true;
                        }
                    }
                    break;
                case RIGHT:
                    for (int i = cs.cellY; i < cs.cellY + cs.spanY; i++) {
                        if (i > rightEdge.length || rightEdge[i] == cs.cellX) {
                            return true;
                        }
                    }
                    break;
                case TOP:
                    for (int i = cs.cellX; i < cs.cellX + cs.spanX; i++) {
                        if (i > topEdge.length || topEdge[i] == cs.cellY + cs.spanY) {
                            return true;
                        }
                    }
                    break;
                case BOTTOM:
                    for (int i = cs.cellX; i < cs.cellX + cs.spanX; i++) {
                        if (i > bottomEdge.length || bottomEdge[i] == cs.cellY) {
                            return true;
                        }
                    }
                    break;
