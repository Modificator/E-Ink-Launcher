...
public abstract class PagedView extends ViewGroup implements ViewGroup.OnHierarchyChangeListener {
    ...
    private boolean mShowLawnfeedPopup;
    ...
    public void scrollTo(int x, int y) {
        ...
        if (isXBeforeFirstPage) {
            ...
            if (mAllowOverScroll) {
                ...
                if (mIsRtl) {
                    shouldScrollOverlay = shouldScrollOverlay(x - mMaxScrollX);
                    overScroll(x - mMaxScrollX);
                } else {
                    shouldScrollOverlay = shouldScrollOverlay(x);
                    overScroll(x);
                }
                mShowLawnfeedPopup = shouldScrollOverlay && allowLawnfeedPopup();
            }
...
//        public boolean isFullScreenPage = true;
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (child.getVisibility() != GONE) {
                ...
                if (!lp.isFullScreenPage) {
                    ...
                    childHeight = getViewportHeight()- mInsets.top - mInsets.bottom;
                }
...
//                }
    ...
    /**
     * 获取距离拖动 celllayout 最近的 layout
     * 现在是判断拖动的 celllayout 边界超过某个 celllayout 的中间，也就是重合大于一半就是最近的 TODO 换成手指距离屏幕边缘百分之几或者手指移到某个celllayout就交换
     *
     * @return
     */
