...
import cn.modificator.launcher.DumbImportExportTask
import cn.modificator.launcher.LauncherFiles
import cn.modificator.launcher.Utilities
import cn.modificator.launcher.preferences.blockingEdit
    ...
    class LegacyBackup(context: Context) : LawnchairBackup(context, null) {
        ...
        override fun readMeta(): Meta? {
            ...
            if (DumbImportExportTask.getDbBackupFile().exists()) {
                contents = contents or INCLUDE_HOMESCREEN
            }
            if (DumbImportExportTask.getSettingsBackupFile().exists()) {
                contents = contents or INCLUDE_SETTINGS
            }
    ...
    companion object {
        ...
        fun listLocalBackups(context: Context): List<LawnchairBackup> {
            val backupList = getFolder().listFiles { file -> file.extension == EXTENSION }
                    ...
                    ?.map { fromUri(context, it) }
                    ?: Collections.emptyList()
            val legacyBackup = fromUri(context, null)
